...
class Plan(models.Model):
    ...
    prix_mensuel = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    prix_annuel = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    nb_categories_max = models.IntegerField(default=5)  # -1 = illimité
    description = models.TextField(blank=True)
...
class Abonnement(models.Model):
    TYPE_CHOICES = [('mensuel', 'Mensuel'), ('annuel', 'Annuel'), ('essai', 'Essai')]
    STATUT_CHOICES = [('actif', 'Actif'), ('expire', 'Expiré'), ('en_attente', 'En attente')]

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    utilisateur = models.OneToOneField('comptes.Utilisateur', on_delete=models.CASCADE, related_name='abonnement')
    plan = models.ForeignKey(Plan, on_delete=models.SET_NULL, null=True, blank=True, related_name='abonnements')
    type = models.CharField(max_length=10, choices=TYPE_CHOICES, default='essai')
    date_debut = models.DateTimeField(default=timezone.now)
    date_fin = models.DateTimeField()
    statut = models.CharField(max_length=15, choices=STATUT_CHOICES, default='actif')
    ...
    def nb_categories_autorisees(self):
        return self.plan.nb_categories_max if self.plan else 5

    def __str__(self):
        return f"{self.utilisateur} - {self.type} ({self.statut})"

class Paiement(models.Model):
    STATUT_CHOICES = [('en_attente', 'En attente'), ('confirme', 'Confirmé'), ('echoue', 'Échoué')]
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
